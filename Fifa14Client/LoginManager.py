...
class LoginManager(object):
    ...
    def login(self):
        ...
        headers = {'Content-Type': 'application/x-www-form-urlencoded','Host':'signin.ea.com','Origin':'https://signin.ea.com'}
        cookies = {'JSESSIONID':self.JSESSIONID}
        payload = {'email': self.email, 'password': self.password, '_rememberMe':'on','rememberMe':'on','_eventId':'submit','facebookAuth':''}
        ...
        next_loc = r.headers['location']#http://www.easports.com/fifa/football-club/login_check?state=
        headers = {'Content-Type': 'application/x-www-form-urlencoded'}
        cookies = {'futweb':self.FUTWEB,'EASFC-WEB-SESSION':self.EASFC_WEB_SESSION,'hl':'uk','XSRF-TOKEN':self.XSRF_TOKEN}
        r = requests.get(next_loc,headers = headers,cookies = cookies,allow_redirects = False)
        self.FUTWEB =  r.cookies['futweb']
        ...
        headers = {'Content-Type': 'application/json','Easw-Session-Data-Nucleus-Id':self.NUCID,'Content-Length':len(self.form_data),'X-UT-Route':'https://utas.fut.ea.com:443'}
        cookies = {'futweb':self.FUTWEB,'EASFC-WEB-SESSION':self.EASFC_WEB_SESSION,'hl':'uk','XSRF-TOKEN':self.XSRF_TOKEN,'device_view':'not_mobile'}
        r = requests.post('http://www.easports.com/iframe/fut/p/ut/auth',headers = headers, cookies = cookies,data = self.form_data)
        ...
        r = requests.post('http://www.easports.com/iframe/fut/p/ut/game/fifa14/phishing/validate',headers = headers, cookies = cookies,data = payload)
    ...
    def is_logged_in(self):
        '''Makes a request for the amount of coins that the account has'''
        '''Returns True if it can get the coin amount, False if it can't'''
        return True
